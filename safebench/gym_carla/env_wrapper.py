...
class VectorWrapper:
    ...
    def __init__(
        self, env_params, scenario_config, world, birdeye_render, display, logger
    ):
        self.logger = logger
        self.world = world
    ...
    def step(self, ego_actions, scenario_actions):
        ...
        for e_i in range(self.num_scenario):
            if not self.finished_env[e_i]:
                processed_action = self.env_list[e_i]._postprocess_action(
                    ego_actions[action_idx]
                )
                # TODO: pre-process scenario action
                self.env_list[e_i].step_before_tick(
                    processed_action, scenario_actions[action_idx]
                )
                action_idx += 1

        if hasattr(self, "pre_tick_callback"):
            self.pre_tick_callback()
...
class ObservationWrapper(gym.Wrapper):
    ...
    def step_before_tick(self, ego_action, scenario_action):
        self._env.step_before_tick(
            ego_action=ego_action, scenario_action=scenario_action
        )
